...
def send_data():
    ...
    id = data['id']
    print(f"Received ID: {id}")  # Thêm thông báo log
    ...
    try:
        ...
        print(f"Sensor nodes: {sensor_nodes}")  # Thêm thông báo log
        if not sensor_nodes:
...
def user_dashboard():
    ...
    if request.method == 'POST':
        # Kiểm tra điều khiển LED
        if 'led_control' in request.form:
            node_id = request.form['led_control']  # Lấy ID từ nút điều khiển LED
